/**
 * SponsorActivationPage — /sponsor/activate?token=<token>
 *
 * Sponsor-scope analogue of AccountActivationPage. Reads `token` from the URL
 * query string, lets the sponsor user set a password, then calls
 * POST /api/v1/sponsor/auth/activate. On success the user signs in at /signin.
 *
 * This route MUST exist as a public route — the activation link in the
 * invitation email points here; without it the URL falls through to the
 * protected /sponsor/:studyId route and bounces to sign-in.
 */
...
export default function SponsorActivationPage() {
      ...
      <div className={styles.page}>
        <div className={styles.card}>
          ...
          <p className={styles.sub}>
            ...
            your email, or contact your CRO administrator for a new invitation.
          </p>
    ...
    return (
      <div className={styles.page}>
        <div className={styles.card}>
          ...
          <p className={styles.sub}>
            Your sponsor account has been activated successfully. You can now sign
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await sponsorAuthService.activate({ token, password, confirmPassword });
      setDone(true);
    } catch (err) {
      const msg = typeof err === 'string' ? err : (err?.message ?? '');
      if (/expired/i.test(msg)) {
        setApiError(
          'This activation link has expired. Please contact your CRO administrator to request a new invitation.',
        );
      } else if (/invalid|already used|not found/i.test(msg)) {
        setApiError(
          'This activation link is invalid or has already been used. Please contact your CRO administrator for a new invitation.',
        );
      } else {
        setApiError(msg || 'Activation failed. Please try again or contact your CRO administrator.');
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <div className={styles.page}>
      <div className={styles.formCard}>
        ...
        <div className={styles.header}>
          ...
          <h1 className={styles.title}>Activate your sponsor account</h1>
